...
func NewGenerator(eventRecorder events.Recorder, kubeconfig *rest.Config, clients *client.Clients, listers *client.Listers, featureGates featuregates.FeatureGate) *Generator {
	return &Generator{
...
func (g *Generator) List(cr *imageregistryv1.Config) ([]Mutator, error) {
	driver, err := storage.NewDriver(&cr.Spec.Storage, g.kubeconfig, &g.listers.StorageListers, g.featureGates)
...
func (g *Generator) storageReconfigured(
	...
	prev, err := storage.NewDriver(&regCfg.Status.Storage, restCfg, &listers.StorageListers, g.featureGates)
	if err != nil {
		return false
	}
	cur, err := storage.NewDriver(&regCfg.Spec.Storage, restCfg, &listers.StorageListers, g.featureGates)
	if err != nil {
		return false
	}
...
func (g *Generator) Apply(cr *imageregistryv1.Config) error {
	...
	// Spec.Storage.ManagementState if the storage did not do it already.
...
func (g *Generator) Remove(cr *imageregistryv1.Config) error {
	...
	driver, err := storage.NewDriver(&cr.Status.Storage, g.kubeconfig, &g.listers.StorageListers, g.featureGates)
	if err == storage.ErrStorageNotConfigured {
		return nil
	} else if err != nil {
		return err
