...
func RunOperator(ctx context.Context, kubeconfig *restclient.Config) error {
	...
	featureGates, err := getFeatureGate(ctx, configInformers, eventRecorder)
	if err != nil {
		return err
	}
...
func getFeatureGate(ctx context.Context, configInformers configinformers.SharedInformerFactory, eventRecorder events.Recorder) (featuregates.FeatureGate, error) {
